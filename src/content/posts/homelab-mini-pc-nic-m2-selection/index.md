...
description: "미니PC 비교표의 CPU와 RAM은 기준선일 뿐입니다. 84GB 복원에 88분이 걸린 경험을 바탕으로 NIC, 스위치, M.2 슬롯, 여유 공간을 하나의 데이터 경로로 놓고 후보를 거르는 방법을 정리했습니다."
...
복원 데이터는 여러 구간을 차례로 지나 목적지에 도착합니다.
...
NIC가 두 개면 관리 트래픽과 스토리지 트래픽을 분리하거나, 서로 다른 네트워크에 연결하거나, 향후 장애 대응 방식을 설계할 여지가 생깁니다. 그러나 포트가 두 개일 때 다음은 별도 설계가 있어야 생깁니다.
...
슬롯 두 개가 주는 것은 배치 선택지까지입니다. RAID도, 복제도, 백업도 별도 계층입니다. 두 SSD는 같은 메인보드, 전원, 냉각, 노드 장애를 공유합니다. 노드가 사라지면 두 디스크가 함께 사라질 수 있습니다.
...
이 사양은 보편적인 추천의 근거라기보다 제 환경의 결론입니다. 이전 NAS에서 확인한 CPU 병목을 넘기고, 유선 복원 경로를 만들고, 스토리지 분리와 확장 가능성을 남기는 제 환경의 결정이었습니다. 현재 판매가와 재고도 이 글의 판단 근거에서 제외했습니다.

전력 소비, 온도, 팬 소음은 24시간 운영에서 중요한 항목입니다. 그러나 후보를 같은 조건에서 측정한 기록이 없어 이번 비교 기준과 M5 Ultra 선택 근거에서는 제외했습니다.
...
이 선택 기준이 실제 3노드 구성과 복원 리허설로 이어진 과정은 [홈랩 쿠버네티스 구축기 2편: 3노드여야 하는 이유, 그리고 빈 결과의 함정](/posts/three-node-etcd-quorum-context/)에서 확인할 수 있습니다.
