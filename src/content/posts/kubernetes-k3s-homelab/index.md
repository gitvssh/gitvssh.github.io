...
description: "컨테이너 런타임, Kubernetes 제어 시스템, 컨트롤 플레인, 워커, 상태 저장 계층의 역할을 구분하고 자신의 홈랩에 k3s가 필요한지 판단한다."
...
draft: false
---
컨테이너 하나를 띄우는 데 성공하고 나면 다음 질문이 생깁니다. “이미 잘 실행되는데 Kubernetes는 왜 필요한가?”

짧게 답하면, **컨테이너 런타임은 컨테이너를 실행하고 Kubernetes는 여러 컨테이너와 노드의 원하는 상태를 계속 맞춥니다.** k3s는 이 Kubernetes 운영 모델을 없앤 별도 도구가 아니라, 설치와 기본 구성요소를 한데 묶은 Kubernetes 배포판입니다.

글·해설: 다메카솔

## 핵심 내용

- 컨테이너 런타임은 컨테이너를 실제로 실행합니다. Kubernetes는 API에 선언한 원하는 상태와 현재 상태의 차이를 계속 줄입니다.
- 컨트롤 플레인은 상태를 저장하고 배치·조정을 결정합니다. 워커 노드는 kubelet과 런타임으로 Pod를 실행합니다.
- k3s는 설치와 기본 구성의 부담을 낮추지만, 데이터스토어·네트워크·영구 스토리지·백업·업그레이드 책임까지 없애지는 않습니다.
- 홈랩에서는 “가볍다”보다 학습 목표, 노드 수, 가동시간, 운영에 쓸 수 있는 시간을 먼저 봐야 합니다.

![카솔이 컨테이너 실행 층과 Kubernetes 제어 층을 비교하는 표지](./cover.webp)
...
이 경계를 그어야 자가 복구도 정확히 이해할 수 있습니다. Kubernetes는 실패한 컨테이너를 재시작하거나 사라진 복제본을 대체할 수 있습니다. 하지만 같은 결함을 가진 새 컨테이너를 다시 띄우는 것만으로 애플리케이션 버그가 고쳐지지는 않습니다. 사용할 수 없어진 스토리지와 손상된 데이터도 별도의 복구 설계가 필요합니다.

## 원하는 상태를 계속 맞추는 제어 시스템

![원하는 Pod 상태와 현재 상태의 차이를 컨트롤러가 감지하고 대체 Pod로 줄이는 흐름](./page-02.webp)

Kubernetes를 이해하는 가장 짧은 길은 `spec`과 `status`를 나눠 보는 것입니다. 사용자는 객체의 `spec`에 원하는 상태를 적습니다. 컨트롤 플레인은 관찰한 현재 상태를 `status`로 갱신하고, 둘 사이에 차이가 생기면 컨트롤러가 다시 맞추려고 움직입니다.
...
모든 구성요소를 서로 다른 물리 서버에 둬야 한다는 뜻은 아닙니다. 단일 노드 학습 클러스터에서는 한 장비가 server와 worker 역할을 함께 맡을 수 있습니다. 역할의 논리적 경계와 물리 장비의 수를 같은 것으로 생각하지 않는 편이 좋습니다.
...
이 숫자에는 사용자 워크로드가 쓸 자원이 포함되지 않습니다. 이미지 다운로드와 압축 해제는 CPU와 디스크 IO를 사용하고, 컨트롤 플레인 변경이 잦거나 Operator가 많으면 server 부하도 커집니다. 데이터스토어와 컨테이너 이미지 저장소, 워크로드 볼륨이 같은 느린 디스크를 다투면 RAM이 남아도 클러스터가 답답해질 수 있습니다.
...
학습 목적이라면 의미가 있습니다. Deployment, Service, Ingress, PersistentVolume, Operator 같은 Kubernetes 개념을 작은 환경에서 익힐 수 있습니다. 다만 노드 한 대의 장애를 견디는 고가용성까지 얻는 것은 아닙니다.
...
아닙니다. 클러스터 데이터스토어 백업, 애플리케이션 데이터 백업, 영구 볼륨 복제는 서로 다른 문제입니다. Pod가 다시 만들어졌다는 사실만으로 데이터가 복구됐다고 볼 수 없습니다.
