...
긴 작업에서 필요한 기억은 사진을 많이 저장하는 일과 같지 않습니다. 정책은 지금 행동을 고르는 데 어떤 과거가 중요한지 구분해야 합니다.
...
이 설계에서는 한 시점의 추론 비용이 누적 context 길이에 따라 커지지 않습니다. Transformer가 긴 KV cache를 계속 읽는 방식과 달리 고정 크기 상태를 전달하기 때문입니다. 전체 모델이 가볍다는 뜻은 아닙니다. 논문 구현은 GR00T N1.7의 16개 DiT 층에 TTT 층을 더해 action head 규모를 538M parameter에서 약 690M으로 늘렸습니다.
...
RoboTTT의 실무적 아이디어는 로봇에게 과거 화면을 끝없이 붙이는 데 있지 않습니다. 다음 행동에 필요한 정보를 **갱신 가능한 내부 상태**로 압축하도록 정책 자체를 학습한 점이 더 중요합니다. 언어 모델의 긴 문맥처럼 로봇에도 context length가 하나의 scaling axis가 될 수 있다는 주장도 이 설계와 실제 로봇 결과에서 나옵니다.

다만 적용 판단은 네 질문으로 좁혀야 합니다.
...
이 글의 만화 이미지는 AI로 생성했습니다. 논문 속 도표·표·사진은 복제하지 않고 의미를 새로 구성해 그렸습니다.
