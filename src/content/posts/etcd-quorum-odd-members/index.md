...
description: "과반은 floor(n/2)+1, 장애 허용 수는 n에서 과반을 뺀 값입니다. 1·2·3·4·5멤버를 직접 계산해 홀수 구성이 유리한 이유와 멤버를 추가·교체할 때 생기는 위험을 짚습니다."
...
draft: false
---
새 홈랩의 k3s 제어면을 구성할 때 두 대와 세 대를 비교했습니다. 두 대도 한 대보다 안전해 보였지만, 두 장비가 모두 etcd 투표 멤버라면 한 대가 멈춘 뒤 새 상태를 결정할 수 없습니다.

제가 세 대를 고른 이유도 이 한 칸의 차이였습니다.

세 번째 멤버가 들어와야 과반 계산상 한 대 장애 뒤에도 제어면 합의를 이어 갈 조건이 생겼습니다. 장비 수를 정할 때는 “홀수가 좋다”는 문장보다 현재 투표 멤버 수의 과반과 장애 허용 수를 먼저 계산해야 합니다.
...
etcd는 Kubernetes의 원하는 상태를 저장하는 일관된 key-value store입니다. 여러 멤버가 있을 때 새 상태를 확정하려면 투표 멤버의 과반이 동의해야 합니다.

한 멤버가 혼자 다른 값을 확정하지 못하게 막는 대신, 과반이 없는 상황에서는 새 쓰기를 진행하지 않는 쪽을 택합니다.
...
이 계산이 성립하는 범위는 두 노드가 모두 투표 멤버일 때까지입니다. 다음을 먼저 구분해야 합니다.
...
이 글의 계산은 **etcd 투표 멤버**에만 적용됩니다. K3s agent를 열 대 추가해도 embedded etcd 투표 수는 그대로입니다. 반대로 server 역할을 분리했다면 모든 server가 etcd 멤버인 것도 아닐 수 있습니다.

3멤버와 4멤버도 비교해 보겠습니다. 3멤버는 과반 2, 장애 허용 1입니다. 네 번째 멤버를 추가하면 과반은 3으로 올라가지만 장애 허용은 여전히 1입니다. 네 번째 장비는 용량이나 다른 역할에 도움을 주고, etcd의 장애 허용 수는 그대로 1에 머뭅니다.
...
제 홈랩의 목표는 한 대가 멈춘 뒤에도 embedded etcd 제어면이 합의를 이어 가는 것이었습니다. 그래서 투표 멤버 두 대가 아니라 세 대를 선택했습니다. 네 대로 늘려도 장애 허용 수는 그대로 1이므로, 장비 수보다 데이터스토어 역할과 허용할 장애 수를 먼저 적는 것이 판단 기준이 됐습니다.
...
이 개념이 실제 운영 판단으로 이어진 장면은 [홈랩 쿠버네티스 구축기 2편: 3노드여야 하는 이유, 그리고 빈 결과의 함정](/posts/three-node-etcd-quorum-context/)에서 확인할 수 있습니다.
