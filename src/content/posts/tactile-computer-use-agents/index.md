...
description: "화면의 버튼을 눌렀는데 빗나갔는지 비활성인지 알 수 없다면 좌표만 본 것입니다. Tactile은 접근성 정보와 OCR, 시각 제어를 단계적으로 결합해 좌표를 행동 가능한 대상 객체로 바꿉니다. 평가의 한계까지 정리했습니다."
...
접근성을 잘 만든 소프트웨어는 화면 읽기 도구와 키보드 사용자를 돕습니다. 이 논문은 같은 의미 구조가 에이전트에게도 안정적인 대상과 행동을 제공할 수 있다고 봅니다. 접근성 메타데이터가 좋을 때 보장되는 범위는 대상 선택의 정확도까지입니다. 의미·좌표·시각을 단계적으로 쓰고, 성공 확인과 사용자 승인 경계를 별도로 설계해야 합니다.
