...
이 결과가 향하는 곳은 도구의 사용 여부보다 검토 절차입니다. AI가 만든 코드도 기능 테스트와 별도로, 해당 API의 보안 기준으로 다시 확인해야 한다는 뜻입니다.
...
둘 다 코드를 실행해 보는 것 이상의 확인을 요구합니다. 인증서나 호스트를 제대로 확인하지 않거나, OAuth의 요청·응답을 묶는 보호 장치를 빠뜨려도 겉으로는 로그인과 통신이 동작할 수 있기 때문입니다.
...
보안 평가에 포함된 구현 가운데 연구가 정한 두 과제의 기준을 모두 만족한 완전히 안전한 구현은 없었습니다. 이 결과가 말하는 범위는 이 실험의 과제와 참가자까지입니다. **이 도구 버전, 두 API 과제, 이 표본과 평가 기준에서 나온 결과**입니다.

## 개발자는 보안 문제를 얼마나 알아챘나

![개발자가 기능 요구만 입력하고 자신의 코드를 안전하다고 생각하지만 실제 검토에서 경고를 발견하는 장면](./page-04.webp)

코드가 작동해도 오용이 남았다면, 남는 질문은 개발자 자신이 그 위험을 알아챘는지입니다. 참가자들은 보안도 평가 대상이라는 사실을 미리 알고 있었습니다. 그런데 Copilot 대화에서 의도적으로 보안 문제를 제기한 참가자는 44명 중 2명이었습니다. 보안 관련 단어를 쓴 참가자는 7명이었지만, 그중 5명은 과제 문구를 그대로 복사한 경우였습니다.
...
따라서 보안을 명시적으로 묻는 일은 첫 단계로서 유용하고, 그 효과는 거기까지입니다. 재프롬프트 결과도 원래 참가자가 수행한 실험 결과가 아니라 연구진의 후속 분석이라는 점을 구분해야 합니다.

## 다메카솔의 실무 해석

저는 이 결과를 도구의 문제라기보다 검토 절차의 문제로 읽습니다. 다음 세 단계는 논문이 효과를 검증한 방법론이 아니라, 제가 연구 결과를 팀의 코드 검토 절차로 옮겨 본 해석입니다.
