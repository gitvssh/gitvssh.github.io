...
description: "CPU 사용률, load average, 실행 대기, CPU PSI, 컨테이너 throttling, 워크로드 동시성을 구분하고 같은 시간대의 신호로 CPU 병목 가설을 검증한다."
...
draft: false
---
Synology NAS에 학습용 k3s를 올려 쓰던 때였습니다. RAM은 32GB라 여유가 있었지만 CPU 코어가 부족했고, Airflow DAG는 반복해서 실패했습니다. 메모리 그래프만 보면 서버는 넉넉해 보였는데 실제 작업은 끝까지 가지 못했습니다.

이 경험에서 얻은 답은 간단합니다. **남는 RAM은 메모리 용량에 여유가 있다는 뜻이지, CPU가 작업을 제때 처리하거나 실행 대기열이 짧다는 뜻은 아닙니다.** CPU 병목을 확인하려면 사용률 하나가 아니라 같은 시간대의 실행 대기, pressure, 컨테이너 throttling, 실제 동시 실행 수를 함께 봐야 합니다.
...
- RAM 여유와 CPU 처리 여유는 서로 다른 질문입니다.
- CPU 사용률은 얼마나 일했는지를 보여 주지만, 얼마나 기다렸는지는 load·실행 대기·CPU PSI로 보완해야 합니다.
- Kubernetes의 CPU `request`와 `limit`은 역할이 다릅니다. 호스트 CPU가 남아도 컨테이너는 limit에 걸려 throttling될 수 있습니다.
- Airflow의 전체·DAG별·Pool 동시성 상한은 겹쳐 작동합니다. 실제로 동시에 실행된 task 수를 확인해야 합니다.
- 같은 입력에서 한 변수만 바꾼 전후 비교가 CPU 경합 가설을 가장 빠르게 좁힙니다.

![메모리 수조에는 빈 공간이 많지만 좁은 CPU 관문 앞에는 작업이 길게 줄 선 모습을 카솔이 살피는 표지](./cover.webp)
...
반대 상황도 가능합니다. CPU가 한가로워 보여도 프로세스가 디스크나 네트워크 응답을 기다리면 서비스는 느립니다. 그래서 “RAM이 남는다” 다음 질문은 “그럼 CPU인가?”가 아니라 “작업은 지금 어디에서 기다리는가?”여야 합니다.
...
CPU 사용률은 관측 구간에 CPU가 실제로 일한 비율입니다. Kubernetes Metrics API의 CPU 값도 커널의 누적 CPU 카운터를 일정 시간 구간의 평균으로 계산합니다. 순간 최고값이 아니라 `window` 안의 평균이라는 점을 기억해야 합니다.
...
cgroup v2 환경에서는 `cpu.stat`에 `nr_throttled`와 `throttled_usec` 같은 필드가 있습니다. 문제 작업을 실행하는 동안 이 값이 얼마나 늘었는지 보면 제한의 영향을 좁힐 수 있습니다. 다만 실제 cgroup 파일 경로는 운영체제, Kubernetes 배포판, 컨테이너 런타임에 따라 달라집니다. 인터넷 예시의 경로를 그대로 복사하기보다 현재 노드가 cgroup v2인지와 해당 컨테이너의 경로를 먼저 확인해야 합니다.
...
### CPU 사용률이 100%면 프로세스를 재시작해야 하나요?

100%라는 사실만으로는 부족합니다. 처리 건수, 읽은 바이트, 완료 stage 같은 진행 지표가 늘고 있다면 계산이 진행 중일 수 있습니다. 진행량이 멈췄는지와 대기가 어디에서 생기는지를 먼저 확인하세요.

### CPU limit을 없애면 해결되나요?

limit이 실제 원인이고 노드에 여유가 있다면 throttling은 줄 수 있습니다. 하지만 한 workload가 노드를 독점해 다른 서비스의 지연을 키울 수도 있습니다. limit 변경 전후의 throttling, 노드 pressure, 다른 Pod의 영향을 함께 검증해야 합니다.
