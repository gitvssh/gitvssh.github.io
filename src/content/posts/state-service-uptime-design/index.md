...
description: "애플리케이션이 24시간 떠 있어도 DB나 오브젝트 스토리지가 밤에 꺼지면 기능은 멈춥니다. 사용자 기능 하나의 의존성 그래프를 그려 가동시간과 복구 역할에 맞는 배치 계층을 나누는 설계법입니다."
...
처음에는 메타데이터 DB만 상시 가동 장비로 옮기는 안을 검토했습니다. 하지만 task의 저장 경로는 여전히 NAS의 S3 호환 오브젝트 스토리지를 향하고 있었습니다. DB만 옮겨도 밤에는 저장 경로가 끊기는 구조였습니다.

저는 이때부터 장비별 전원 시간이 아니라 DAG 한 번이 지나가는 전체 경로를 기준으로 옮길 대상을 판단했습니다.

**애플리케이션의 실질 가동시간은 필수 의존성이 함께 사용 가능한 시간으로 제한됩니다.** 24시간 기능이 필요하다면 DB와 오브젝트 스토리지, 이름 해석과 인증처럼 그 기능이 통과하는 경로도 같은 시간 동안 살아 있어야 합니다.

글·해설: 다메카솔

> 당시의 정확한 장애 타임스탬프와 로그는 회수하지 못했습니다. 확인 가능한 것은 야간에 꺼지는 NAS에 메타데이터 DB와 오브젝트 스토리지가 함께 있었고, DB만 옮기는 안으로는 task 저장 경로가 남는다는 설계 맥락까지입니다. 아래 절차는 현재 환경에서 같은 구조를 찾기 위한 방법입니다.

## 가동시간은 겹치는 시간창입니다

![소비자는 하루 종일 켜져 있지만 DB와 스토리지는 일부 시간만 켜져 있어 실제 동작 시간이 겹치는 구간으로 좁아지는 도식](./page-01.webp)

애플리케이션 프로세스가 하루 종일 살아 있어도 요청을 처리하려면 다른 서비스가 필요합니다. 로그인에는 인증과 사용자 DB가, 파일 업로드에는 이름 해석과 메타데이터 DB와 오브젝트 스토리지가 필요할 수 있습니다.

이 가운데 하나라도 필요한 순간에 사용할 수 없다면 프로세스는 살아 있어도 기능은 중단됩니다.

따라서 가동시간은 서버별 숫자가 아니라 겹치는 시간창의 문제입니다. 사용자 기능 하나가 통과하는 필수 의존성들의 **공통 시간창**을 찾는 문제입니다. 소비자는 24시간 켜져 있고 DB가 주간에만 켜진다면 해당 기능의 가동시간은 DB의 시간창을 넘지 못합니다.

이 기준이 적용되는 대상은 소비자가 24시간 필요로 하는 의존 서비스까지입니다. 낮에만 쓰는 개발 환경은 의존성도 그 시간에 맞추면 됩니다. 목표는 무조건적인 상시 가동이 아니라, 독자가 기대하는 사용 시간과 실행 경로의 가동시간을 일치시키는 것입니다.
...
## DB만 옮기는 안으로는 왜 부족했을까

![메타데이터 DB는 상시 클러스터로 옮겼지만 오브젝트 저장소가 야간에 꺼지는 NAS에 남아 실행 경로가 여전히 끊기는 장면](./page-03.webp)

홈랩에서 처음 검토한 안은 메타데이터 DB만 상시 가동 장비로 옮기는 것이었습니다. 그러나 task가 원본을 쓰는 경로를 따라가 보니 야간에 꺼지는 NAS의 오브젝트 스토리지가 남았습니다. 저는 DB 하나의 위치보다 DAG가 끝까지 통과해야 하는 경로를 먼저 옮겨야 한다고 판단했습니다.
...
이 사례에서 마이그레이션의 완료 조건은 “DB가 새 서버에서 실행됨”보다 길어야 합니다. **새 입력이 들어와 최종 결과를 다시 읽을 때까지 이전 실행 경로를 전혀 사용하지 않는 것**까지 확인해야 합니다. 로그와 설정 검색, 네트워크 관측, 실제 end-to-end 요청을 함께 쓰는 이유입니다.
...
StatefulSet은 Pod에 안정적인 네트워크 식별자와 지속 스토리지를 연결하고 순서가 필요한 배포·확장을 돕습니다. 그러나 스토리지 자체, 데이터베이스 복제와 장애 조치 정책, 백업은 각각 따로 설계해야 하는 계층입니다. StatefulSet을 썼다는 사실만으로 DB가 고가용성이라고 말할 수 없습니다.
...
아래 여섯 단계는 문서가 없는 홈랩에서 가동시간 설계의 초안을 만드는 순서입니다. 호출 경로와 장애 경계를 적은 뒤 실제 중단 시험은 다음 단계로 분리합니다.
...
## 중단 시험과 실제 기능 검증은 따로 계획합니다

![서로 다른 노드의 복제본도 같은 전원과 스토리지를 공유하면 함께 중단될 수 있으며 남성 카솔이 실제 기능과 복구 순서를 시험하는 마무리 장면](./page-05.webp)

Pod phase가 `Running`이면 Pod가 노드에 배치됐고 모든 컨테이너가 생성됐으며, 적어도 하나가 실행 중이거나 시작·재시작 중이라는 뜻입니다. 모든 컨테이너가 `Ready`이거나 사용자 기능이 성공했다는 뜻은 아닙니다.

`Ready` 조건도 Kubernetes가 설정된 readiness 검사를 통과했다고 판단한 결과입니다. 검사 범위에 DB와 오브젝트 저장이 없다면 실제 사용자 경로의 성공까지 보장하지 않습니다. 가동시간 검증은 같은 입력을 넣고 최종 결과를 다시 읽는 합성 점검으로 닫아야 합니다.
...
초안 다음에는 별도의 검증 단계가 필요합니다. 실제 중단 시험은 영향 범위, 관측 방법, 백업과 복구 순서, 중단 권한을 정한 별도 계획으로 수행합니다. 작은 테스트 namespace와 합성 데이터로 시작하고, 전원을 끄기 어렵다면 제한된 범위의 방화벽이나 Service endpoint 차단으로 실패 경로부터 관찰할 수 있습니다.

제가 이 홈랩에서 먼저 고친 것은 장비 수가 아니라 배치 기준이었습니다. DAG가 지나가는 경로를 그린 뒤에야 상시 가동할 서비스와 예약하거나 분리할 장비를 구분할 수 있었습니다.
...
이 설계를 적용하게 된 실제 홈랩 배경은 [홈랩 쿠버네티스 구축기 1편: RAM은 32GB인데 Airflow DAG가 계속 실패한 이유](/posts/why-homelab-kubernetes/)에서 이어집니다.

> **정정 (2026-07-28)**: 초판은 메타데이터 DB를 상시 가동 장비로 실제 옮긴 뒤에도 문제가 남은 것처럼 썼고, Pod의 `Running` 상태를 단순히 컨테이너가 떠 있는 상태로 설명했습니다. 현재 글은 DB 이전이 검토 단계였다는 사실과 Kubernetes Pod phase의 정확한 범위를 반영해 고쳤습니다. 또한 가동시간 설계 초안 작성과 실제 중단 시험을 분리했습니다.
